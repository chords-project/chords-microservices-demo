...
public class TCPReactiveClientConnectionPool implements ClientConnectionManager {
    ...
    public TCPReactiveClientConnectionPool(String address)
    ...
    public void connect() {
        Thread.ofPlatform()
                .start(() -> {
                    for (int i = 0; i < 10; i++) {
                        try {
                            synchronized (TCPReactiveClientConnectionPool.this) {
